...
if config.enabled.bufferline then
  ...
  map("n", "<S-l>", "<cmd>bn<CR>", opts)
end
map("n", "<S-h>", "<cmd>bp<CR>", opts)
...
if config.enabled.toggle_term then
  ...
  map("n", "<leader>tf", "<cmd>ToggleTerm direction=float<cr>", opts) map("n", "<leader>th", "<cmd>ToggleTerm size=10 direction=horizontal<cr>", opts) map("n", "<leader>tv", "<cmd>ToggleTerm size=80 direction=vertical<cr>", opts)
end
...
-- Better_escape plugin does not handle visual mode
map("n", "qf", "<esc>", opts)
map("i", "qf", "<esc>", opts)
map("v", "qf", "<esc>", opts)
